...
void mim(int idx, int e, int val, int i) {
    if(idx == e) {
        sum[i].push_back(val);
        return;
    }
    mim(idx + 1, e, (val + a[idx]) % m, i);
    mim(idx + 1, e, val, i);
}
...
void solve() {
    ...
    rep(i, 0, sum[0].size()) {
        ...
        if(d <= 0) {
            ...
            int idx = upper_bound(sum[1].begin(), sum[1].end(), d - 1) - sum[1].begin();
            --idx;
            if(idx >= 0) {
                ans = max(ans, (sum[0][i] + sum[1][idx]) % m);
            } else {
                ans = max(ans, (sum[0][i] + sum[1].back()) % m);
            }
        }
    }
