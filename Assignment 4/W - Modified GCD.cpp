...
void solve() {
    int a, b, n; cin>>a>>b>>n;
    vi div1 = getFactors(a);
    vi div2 = getFactors(b);
    sort(all(div1));
    sort(all(div2));
    vi intersect;
    set_intersection(div1.begin(), div1.end(), div2.begin(), div2.end(), inserter(intersect, intersect.begin()));
    int l,h;
    rep(i,0,n){
        cin>>l>>h;
        int it = upper_bound(all(intersect),h) - intersect.begin();
        it--;
        if(it == -1 || intersect[it] < l){
            cout<<-1<<endl;
        }else{
